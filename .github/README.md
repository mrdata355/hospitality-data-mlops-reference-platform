...
This directory defines the repository controls used to validate changes, retain evidence, manage dependencies, publish versioned container images, and execute authorized cloud validation.
...
| `workflows/deploy-azure-validation.yml` | manual dispatch in the `azure-validation` environment | OIDC authentication, deterministic artifact generation, ACR build, Azure Container Apps deployment, live API validation, and sanitized evidence retention |
