...
def build_member_month_features() -> pd.DataFrame:
    ...
    base = members[
        ["member_id", "member_tier", "home_market", "member_since_date", "engagement_seed"]
    ].copy()
    base["as_of_month"] = as_of_month
    base["tenure_months"] = (
        (as_of_month.year - base.member_since_date.dt.year) * 12
        + (as_of_month.month - base.member_since_date.dt.month)
    ).clip(lower=0)

    p = points[
        (points.transaction_date >= as_of_month - pd.DateOffset(months=12))
        & (points.transaction_date < as_of_month)
    ].copy()
    p["points_earned_12m"] = np.where(p.transaction_type == "EARN", p.points_amount, 0)
    p["points_redeemed_12m"] = np.where(
        p.transaction_type == "REDEEM", p.points_amount.abs(), 0
    )
    p["points_expired_12m"] = np.where(
        p.transaction_type == "EXPIRE", p.points_amount.abs(), 0
    )
    p_agg = p.groupby("member_id", as_index=False)[
        ["points_earned_12m", "points_redeemed_12m", "points_expired_12m"]
    ].sum()
    ...
    s["booking_lead_days"] = (s.check_in_date - s.booking_date).dt.days.clip(lower=0)
    s_agg = s.groupby("member_id", as_index=False).agg(
        stays_12m=("stay_id", "nunique"),
        room_nights_12m=("room_nights", "sum"),
        last_booking_date=("booking_date", "max"),
        net_room_revenue_12m=("net_room_revenue", "sum"),
        avg_booking_lead_days_12m=("booking_lead_days", "mean"),
    )

    c = cases[
        (cases.case_created_at >= as_of_month - pd.Timedelta(days=90))
        & (cases.case_created_at < as_of_month)
    ].copy()
    c["escalated"] = c.case_priority.eq("ESCALATED").astype(int)
    c_agg = c.groupby("member_id", as_index=False).agg(
        service_cases_90d=("case_id", "nunique"),
        escalated_cases_90d=("escalated", "sum"),
        avg_resolution_hours_90d=("resolution_hours", "mean"),
    )

    feat = (
        base.merge(p_agg, on="member_id", how="left")
        .merge(s_agg, on="member_id", how="left")
        .merge(c_agg, on="member_id", how="left")
    )
    numeric = [
        "points_earned_12m",
        "points_redeemed_12m",
    ...
    feat["days_since_last_booking"] = (
        (as_of_month - feat.last_booking_date).dt.days.fillna(999).clip(lower=0)
    )
    feat["points_utilization_rate"] = (
        feat.points_redeemed_12m / feat.points_earned_12m.replace(0, np.nan)
    ).fillna(0).clip(0, 3)
    feat["expired_share"] = (
        feat.points_expired_12m / (feat.points_earned_12m.abs() + 1)
    ).clip(0, 3)

    risk = 1.6 * (
        0.20
        + 0.008 * feat.days_since_last_booking
        - 0.18 * feat.stays_12m
        - 0.60 * feat.points_utilization_rate
        + 0.30 * feat.escalated_cases_90d
        + 0.12 * feat.service_cases_90d
        - 0.80 * feat.engagement_seed
        + 0.40 * feat.expired_share
    )
    probability = 1 / (1 + np.exp(-risk))
    deterministic_uniform = feat.member_id.map(
        lambda value: int(hashlib.sha256(value.encode("utf-8")).hexdigest()[:8], 16) / 16**8
    )
...
def build_waterfall_features() -> pd.DataFrame:
    ...
    weekly = valid.groupby(["resort_id", "week_start"], as_index=False).agg(
        arrivals=("reservation_id", "nunique"),
    ...
    scaffold = pd.MultiIndex.from_product(
        [resorts.resort_id, full_weeks], names=["resort_id", "week_start"]
    ).to_frame(index=False)
    weekly = scaffold.merge(weekly, on=["resort_id", "week_start"], how="left").fillna(
        {"arrivals": 0, "room_nights": 0, "net_room_revenue": 0}
    )
    weekly = weekly.merge(
        resorts[["resort_id", "market", "capacity_units"]], on="resort_id", how="left"
    )

    marketing["week_start"] = marketing.touch_time.dt.to_period("W-SUN").dt.start_time
    intensity = (
        marketing.groupby(["market", "week_start"], as_index=False)
        .size()
        .rename(columns={"size": "campaign_intensity"})
    )
    ...
    weekly["rolling_mean_4w"] = weekly.groupby("resort_id").arrivals.transform(
        lambda series: series.shift(1).rolling(4).mean()
    )
    weekly["rolling_mean_13w"] = weekly.groupby("resort_id").arrivals.transform(
        lambda series: series.shift(1).rolling(13).mean()
    )
